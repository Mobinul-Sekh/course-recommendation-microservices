...
import { uploadCourses } from '@/lib/courseApi';
...
function UploadCard({ title, description, onUpload, onDownloadTemplate }: { title: string, description: string, onUpload: (file: File) => void, onDownloadTemplate?: () => void }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await uploadCourses(selectedFile, token, (progress) => {
      ...
      if (response.success) {
        toast({
          title: 'Success',
          description: `Successfully uploaded ${response.count || 0} courses`,
          variant: 'default',
        });
        onUpload(selectedFile);
      } else {
        let errorMessage = response.message || 'Failed to upload courses';
        
        if (response.failedRows && response.failedRows.length > 0) {
          errorMessage += ` (${response.failedRows.length} rows failed validation)`;
          console.error('Validation errors:', response.failedRows);
        }
